...
local function handleLevelSelect( event )
	if ( "ended" == event.phase ) then
		...
		if(event.target.id == '1') then
			composer.gotoScene( "game.levels.level1", { effect="crossFade", time=333 } )
			print("Go to: Level 1: Lower Class")
		elseif ( event.target.id == '2') then
			composer.gotoScene( "game.levels.level2", { effect="crossFade", time=333 } )
			print("Go to: Level 2: Middle Class")
		else
			composer.gotoScene( "game.levels.level3", { effect="crossFade", time=333 } )
			print("Go to: Level 3: Upper Class")
		end		
	end
end
...
function scene:create( event )
	...
	local background = display.newImageRect( sceneGroup, "images/screens/imgLvl0.png", 580, 300 )
	...
	local doneButton = display.newImageRect( sceneGroup, "images/buttons/menuBtn.png", 55, 55 )
	doneButton.x = 250
	doneButton.y = 280
